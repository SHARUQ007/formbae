    ...
    <div className="surface p-3 sm:p-4">
      <div className="mb-3 flex flex-col gap-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <p className="text-xs text-zinc-500">{filtered.length} shown</p>
        </div>
        <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center">
          ...
          />
          {query && (
            <button type="button" className="btn btn-muted w-full sm:w-auto" onClick={() => setQuery("")}>
              Clear
            </button>
          )}
        </div>
      ...
      <ul className="space-y-3">
        {filtered.map((e) => (
          <li key={e.exerciseId} className="rounded-xl border border-emerald-100 p-3 text-sm">
            <details>
              <summary className="cursor-pointer list-none rounded-lg p-1">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    <p className="break-words font-medium">{e.name}</p>
                    <p className="text-xs text-zinc-600 sm:text-sm">{e.primaryMuscle} • {e.equipment}</p>
                  </div>
              ...
              <form action={`/api/trainer/exercises/${e.exerciseId}`} method="post" className="mt-3 space-y-3 border-t border-emerald-100 pt-3">
                <div>
                    ...
                    <>
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div>
                          ...
                          <textarea name="cuesText" rows={5} defaultValue={cuePack.cues.join("\n")} required />
                        </div>
                        <div>
                          <label>3 Common Mistakes (one per line)</label>
                          <textarea name="mistakesText" rows={5} defaultValue={cuePack.mistakes.join("\n")} required />
                        </div>
                      </div>
                ...
                <button className="btn btn-secondary w-full sm:w-auto" type="submit">Save Changes</button>
              </form>
            </details>
          </li>
        ))}
